...
Inductive hypothesis: suppose \(n + m\) is a natural number. Wanted: \((n\INC) + m\) is a natural number. By \DEF{2.2.1} \BLUE{\((n\INC) + m = (n + m)\INC\)}, but by inductive hypothesis, \(n + m\) is a natural number, and by Axiom \ref{axm 2.2}, \BLUE{\((n + m)\INC\)} is a natural number, so \BLUE{\((n\INC) + m\)} is a natural number. This closes the induction.
\end{proof}
...
Inductive hypothesis: suppose \(a + b = a + c \implies b = c\), we have to prove \(\BLUE{(a\INC) + b} = \GREEN{(a\INC) + c} \implies b = c\). By \DEF{2.2.1}, \BLUE{LHS} = \((a + b)\INC\), \GREEN{RHS} = \((a + c)\INC\), so \((a + b)\INC = (a + c)\INC\), and by Axiom (of contrapositive of) \ref{axm 2.4}, \(a + b = a + c\), which by inductive hypothesis implies \(b = c\). This closes the induction.
\end{proof}
...
\end{note}
\begin{proof}{(a)}
Let \(a\) be a natural number. Then by \DEF{2.2.11}, \(\BLUE{a + 0} \geq \GREEN{a}\) because \(0\) is a natural number. But by \LEM{2.2.2}, \(\BLUE{a + 0} = \BLUE{a}\), so by Axiom of Substitution (\ref{axm a.7.4}), \(\BLUE{a} \geq \GREEN{a}\).
\end{proof}
...
Inductive hypothesis: Suppose \(a\) satisfies the "at least one" proposition, i.e. at least one of \(a < b\), \(a = b\), or \(a > b\) is true. We have to prove \(a\INC\) satisfies the "at least one" proposition.
    \begin{enumerate}
        ...
        \item Suppose \(a = b\), then \(a\INC = b\INC\) by Axiom of Substitution \ref{axm a.7.4}, and \(b\INC > b\) by \AC{2.2.3}, so \(a\INC > b\) (\BLUE{This is the third (why?)})
...
Let \(n\) be a natural number, \(P(m)\) be a property holding the \RED{implication}. We first define \(Q(m) = P(m + n)\). Then it is trivial that     \begin{center}
    ...
    \end{center}
because of the \RED{implication} of \(P(m)\). Now suppose \(P(n)\) is true. Then \(P(\RED{0} + n)\) is true since \(n = 0 + n\) and by Axiom of Substitution \ref{axm a.7.4} \(P(n) = P(\RED{0} + n)\). But that just implies Q(\RED{0}) is true since we define \(Q(\RED{0}) = P(\RED{0} + n)\). So we have \(Q(0)\) is true and \MAROON{(1)}, they are base case and inductive hypothesis of principle of induction, and by Axiom \ref{axm 2.5}, \(Q(m)\) is true for all natural number \(m\), that is, \(P(m + n)\) is true for all natural number \(m + n\), or for all natural number \(\geq n\). (since \(m + n \geq n\) by \DEF{2.2.11}.)
\end{proof}