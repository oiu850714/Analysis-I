...
\begin{note}
這個公理(axiom scheme)的前半部我看起來好像寫成「對於任意一個\ predicate \(P\) 使得對於所有\ \(x \in A\)，\(P(x)\) 要馬是\ true 要馬是\ false」好像比較準確\dots。原文看起來會感覺是對於每個\ \(x \in A\)，都存在一個\ predicate P，使得\ \(P(x)\) 要馬為\ true 要馬為\ false。這兩個應該是完全不同的意思。參考\href{https://math.stackexchange.com/questions/9542/how-can-i-write-the-axiom-of-specification-as-a-sentence}{這個}。之後的\ \AXM{3.6} 也是同理。
\end{note}
