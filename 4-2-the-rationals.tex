...
\end{align*}
Thus if \(a\) and \(b\) are non-zero (note that \(b\) must be non-zero) then so is \(a \D b\). This is used in \DEF{4.2.11}.
\end{additional corollary}

We now define a new operation on the rationals: reciprocal. See \DEF{4.2.11} (just give the definition a label to make it convenient to refer to).
