...
\begin{additional corollary}[on top of textbook page 30] \label{ac 2.3.1}
...
\begin{note}
這邊會再多額外幾個\ Corollary，主要是為了證明\ \LEM{2.3.2}，其證明「流程」跟加法交換律\ \PROP{2.2.4} 很類似。
\end{note}

\begin{additional corollary} \label{ac 2.3.2}
Let \(n\) be a natural number. Then \(n \X 0 = 0\).
This can be compared to \LEM{2.2.2}.
\end{additional corollary}
...
\end{example}
\begin{note}
上個例子最後一句話，作者在定義加法時有做類似的說明(p. 24 加法定義後面文字)，但是在定義乘法的時候沒有，可能忘了吧。另外指數乘法這裡不會探討太多，會在第四章定義完整數跟有理數後再繼續討論，可參考\ \PROP{4.3.10} and \PROP{4.3.12}。
\end{note}
